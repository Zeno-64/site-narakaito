...
type Atalho = {
  ...
  rotulo: string
  icone: ReactNode
  /** O WhatsApp é o canal de venda: fica preenchido e maior que o resto. */
  principal?: boolean
}
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {atalhos.map((atalho) => (
        <a
          ...
          className={`group relative flex items-center justify-center rounded-full shadow-lg transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ember-300 ${
            atalho.principal
              ? 'h-14 w-14 bg-ember-500 text-bone-100 shadow-ember-900/50 hover:bg-ember-400'
              : 'h-11 w-11 border border-ink-600 bg-ink-950/90 text-bone-300 shadow-ink-950/60 hover:border-ember-500 hover:text-bone-100'
          }`}
        >

...
**Push na `main` não publica.** Subir para o GitHub não muda o ar: o deploy é
manual, e sem estes dois comandos o site fica na versão anterior por tempo
indeterminado.
...
Isto já custou uma confusão: o README chegou a afirmar que a Cloudflare
publicava sozinha a cada push. Não publica — não há workflow em `.github/` e
não há projeto ligado ao repositório. Três commits ficaram na `main` sem
chegar ao ar até alguém rodar o `wrangler deploy`. Se um dia o repositório
for ligado à Cloudflare em **Workers & Pages**, é aqui que essa mudança
precisa ser escrita.
